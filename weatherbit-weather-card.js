...
// #####
  ...
  slotValue(slot,value){
    ...
    var dewpoint = this.config.alt_dewpoint ? html`<li><span class="ha-icon"><ha-icon icon="mdi:coolant-temperature"></ha-icon></span><span id="alt-dewpoint">${this._hass.states[this.config.alt_dewpoint].state}</span></li>` : this.config.entity_dewpoint ? html`<li><span class="ha-icon"><ha-icon icon="mdi:coolant-temperature"></ha-icon></span><span id="dewpoint-text">${this.current.dewpoint}</span><span> ${this.getUOM('temperature')} </span></li>` : ``;
...
// #####
  ...
  get weatherIcons() {
    return {
      ...
      'partlycloudy-day': 'cloudy-day-3',
      'partly-cloudy-night': 'cloudy-night-3',
      'partlycloudy-night': 'cloudy-night-3',
...
// #####
  ...
  get current() {
    ...
    var pressure = this.config.entity_pressure ? this._hass.states[this.config.entity_pressure].state : 0;
    ...
    var dewpoint = this.config.entity_dewpoint ? Math.round(this._hass.states[this.config.entity_dewpoint].state) : 0;
...
style() {
  ...
  var tempTopMargin = this.config.temp_top_margin || "0em";
  ...
  var currentTextLeftPos = this.config.current_text_left_pos || "0em";
  ...
  var forecastHeight = this.config.forecast_height || "10em";
...
// Optional Entities    
      ...
      if (this.config.entity_pop && !this.config.alt_pop) try { root.getElementById("pop-text").textContent = `${(this._hass.states[this.config.entity_pop].state)}` } catch(e) {}
      if (this.config.entity_pop_intensity && !this.config.alt_pop) try { root.getElementById("pop-intensity-text").textContent = ` - ${this._hass.states[this.config.entity_pop_intensity].state} ${this.getUOM('intensity')}` } catch(e) {}
      ...
      if (this.config.entity_daily_summary) try { root.getElementById("daily-summary-text").textContent = `${this._hass.states[this.config.entity_daily_summary].state}` } catch(e) {}
      if (this.config.entity_dewpoint && !this.config.alt_dewpoint) try { root.getElementById("dewpoint-text").textContent = `${this.current.dewpoint}` } catch(e) {}
      if (this.config.entity_precip_1) try { root.getElementById("precip-tommorow-text").textContent = `${this._hass.states[this.config.entity_precip_1].state}` } catch(e) {}
...
// Alt Text
      ...
      if (this.config.alt_dewpoint) try { root.getElementById("alt-dewpoint").textContent = `${this._hass.states[this.config.alt_dewpoint].state}` } catch(e) {}
    }
  }
...
// #####
customElements.define('weatherbit-weather-card', WeatherbitWeatherCard);

...
// #####
  ...
  get current() {
    ...
    var dewpoint = this.config.entity_dewpoint ? this._hass.states[this.config.entity_dewpoint].state : 0;
...
// Forecast blocks
      this.forecast.forEach((daily) => {
        ...
        root.getElementById("fcast-high-" + daily.dayIndex).textContent = `${Math.round(this._hass.states[daily.temphigh].state)}${this.config.old_daily_format ? this.getUOM("temperature") : ""}`;
...
// Optional Entities    
      ...
      if (this.config.entity_pop && !this.config.alt_pop) { root.getElementById("pop-text").textContent = `${(this._hass.states[this.config.entity_pop].state)}` }
      ...
      if (this.config.entity_dewpoint && !this.config.alt_dewpoint) { root.getElementById("dewpoint-text").textContent = `${this.current.dewpoint}` }
...
// Alt Text
      ...
      if (this.config.alt_dewpoint) { root.getElementById("alt-dewpoint").textContent = `${this._hass.states[this.config.alt_dewpoint].state}` }
    }
  }
